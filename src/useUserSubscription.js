    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            ...
            if (currentUser) {
                ...
                const userSubscriptionRefs = await getDocs(collection(doc(db, 'customers', currentUser.uid), 'subscriptions'));
                if (userSubscriptionRefs.docs.length > 0) {
                    const userSubscriptionRef = userSubscriptionRefs.docs[0];
                    if (userSubscriptionRef.exists()) {
                        let subscriptionActivity = userSubscriptionRef.data().items[0].plan.active;
                        let subscriptionPlan = userSubscriptionRef.data().items[0].price.product.name;
                        let subscriptionRenewalSeconds = userSubscriptionRef.data().current_period_end.seconds;
